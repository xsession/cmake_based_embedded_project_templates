...
#define PIXEL_PIN          D5     // GPIO Pin
#define PIXEL_DASH         1     // Binary segment

#define LDR_PIN       A0    // LDR pin
#define DHT_PIN       D2    // DHT Sensor pin
...
const char *ssid     = "TP-Link_5C14";
const char *password = "QGALYRUUG4AL";

WiFiUDP ntpUDP;
// 'time.nist.gov' is used (default server) with +1 hour offset (3600 seconds) 60 seconds (60000 milliseconds) update interval
NTPClient timeClient(ntpUDP, "hu.pool.ntp.org", 7200, 60000); //GMT+5:30 : 5*3600+30*60=19800 // GMT+1 : 1*3600
// NTPClient timeClient(ntpUDP, "hu.pool.ntp.org"); //GMT+5:30 : 5*3600+30*60=19800 // GMT+1 : 1*3600
...
void disp_Dash() {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    for (int j = 0; j < PIXEL_DASH; j++) {
      ...
      Second % 2 == 0 ? strip.setPixelColor(dash, strip.Color(0, 0xff,  0xc3)) : strip.setPixelColor(dash, strip.Color(0, 0,  0));
    }
  }
}
...
void writeDigit(int index, int val) {
  ...
  for (int i = 6; i >= 0; i--) {
    ...
    uint32_t color;
    if (digit & 0x01 != 0) {
      if (index == 0 || index == 1 ) color = strip.Color(0, 0xff,  0xc3);
      if (index == 2 || index == 3 ) color = strip.Color(0, 0xff,  0xc3);
      if (index == 4 || index == 5 ) color = strip.Color(0, 0xff,  0xc3);
    }
    else
